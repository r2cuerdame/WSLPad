...
在左侧选板块，在右侧看内容：一共十四个，从概览一直到警告。表格能用满整个窗口，
...
和防火墙推算出来。当这些事实读不到时，WSLPad 会说*未知*，而不是靠猜。
...
（Streamable HTTP，仅限本机，Bearer 令牌认证），带 29 个 `Get*` 工具 ——
...
正在使用的名称服务器，以及 Windows 适配器下发的服务器。
...
**通过 MCP** —— 以上全部内容，都可以通过 29 个只读 `Get*` 工具拿到。
