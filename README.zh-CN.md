...
**已安装工具** —— 11 个类别共 112 个工具（AI CLI、运行时、包管理器、版本控制、
...
**开发者配置文件** —— 五套推荐工具集（Web、Python、Rust、AI、容器 / Kubernetes）
与上面的已安装工具目录进行对照：已安装多少、缺失多少，以及每个工具为何在列表中的一句说明。
缺失的工具会解析到 WSLPad 已在发行版中看到的包管理器 —— 与 Discover 准备的相同的
apt / npm / cargo / Homebrew / snap 命令 —— 若都没有则交给 Discover。**准备缺失的安装**
只会在 Console 中放入一行可审阅的命令，按下 Enter 之前什么都不会运行。自定义配置文件
只是一个名称加上从目录中挑选的工具，保存在设置中 —— WSLPad 不会变成包管理器。
