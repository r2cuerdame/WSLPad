...
在左侧选板块，在右侧看内容：概览、实时 CPU/内存/磁盘、重要路径、配置文件、
自动检测到的开发工具、专门的 Hermes 板块、环境变量（密钥已脱敏）、进程、服务、
端口和警告。表格能用满整个窗口，而不是挤在一张小卡片里；左侧列表还带实时角标
（进程数、开放端口、警告数、Hermes 状态）。

**端口**板块会同时显示每个端口的两侧：WSL 侧的监听标记为 `WSL`，确实能从
Windows 访问时则标记为 `WSL + Windows`，并列出占用它的 Windows 进程（在 NAT
网络模式下通常是 `wslrelay`）。仅 Windows 的监听也会一并列出，可以关掉不看。
当无法读取主机端口列表时，WSLPad 会如实说明，而不是直接断言“无法访问”。
...
（Streamable HTTP，仅限本机，Bearer 令牌认证），带 23 个 `Get*` 工具 ——
...
- 工具检测目录仍是最初的 18 项；更大、带分类的目录已排在 0.1.2
...
下一步（0.1.2）：大幅扩充并分类的工具目录、Explorer 窗格中按发行版区分的图标，
以及回收站还原界面。再往后：按发行版的 Console 配置、服务日志查看器、ARM64
构建、已签名的安装程序。
