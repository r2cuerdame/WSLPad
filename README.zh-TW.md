...
左邊挑一個區塊，右邊就讀它 —— 從總覽到警告，一共十四個。表格用的是整個視窗，
...
（Streamable HTTP、僅限 localhost、Bearer 權杖驗證），共 29 個 `Get*` 工具 ——
...
只有這台電腦連得到、只有 WSL 內部連得到、哪裡都到不了，或未知。
...
**透過 MCP** —— 以上全部，都能透過 29 個唯讀的 `Get*` 工具取得。
