...
左邊挑一個區塊，右邊就讀它 —— 從總覽到警告，一共十三個。表格用的是整個視窗，
而不是擠在一張小卡片裡，左側清單還帶著即時徽章。完整清單在
[下方](#你實際看得到什麼)；其中有三個區塊值得特別點出來，因為它們回答的是
WSL 自己不肯回答的問題：
...
`WSL + Windows`（並附上持有它的 Windows 處理程序 —— 在 NAT 網路模式下通常是
`wslrelay`）。只存在於 Windows 的監聽者也會列出來。當主機端的連接埠表讀不到時，
WSLPad 會照實說*未知*，而不是逕自宣稱「無法從 Windows 存取」。
...
（Streamable HTTP、僅限 localhost、Bearer 權杖驗證），共 26 個 `Get*` 工具 ——
...
以及給 Windows 用的 `\\wsl.localhost\…` 路徑。

**資源** —— 即時 CPU %、記憶體已用／總量、交換空間、`/`、`/home` 與 `/mnt/c` 的
磁碟使用量、平均負載、處理程序數量。另外還有**記憶體對帳**：Windows 記憶體、
WSL 記憶體上限（以及這個上限是你設的還是 WSL 自己算的）、Windows 目前為這台
虛擬機器保留了多少，還有 Linux 內部的已用／快取／可用／交換空間分佈 —— 於是
「vmmem 吃掉 7 GB」就變成了「其中大部分是可以還回來的頁面快取」。

**磁碟映像** —— `ext4.vhdx` 在你的 Windows 磁碟上實際放在哪裡、映像大小、真正
配置了多少、是不是疏鬆檔案、發行版內部的檔案系統大小與使用量，以及有多少可回收。

**WSL 設定** —— `.wslconfig` 與 `/etc/wsl.conf` 裡的每一個鍵，附上宣告值、實際
生效的值、它是從哪裡來的，以及一個判定：已生效、需要重新啟動、未設定、未知的鍵
（打錯字）、寫錯區段，或這個組建不支援。也包括實際執行中的網路模式與你要求的
那個模式，以及當虛擬機器比你最後一次編輯還早啟動時出現的橫幅提示。

**重要路徑** —— `$HOME`、`/etc`、`/usr/local/bin`、`~/.local/bin`、`~/.config`、
`~/.cache`、`~/.ssh`、`~/.hermes`，還有從 Linux 這邊看到的 Windows 使用者設定檔
目錄 —— 每一項都標示存不存在，並同時給出 Linux 與 Windows 兩種寫法。
...
uv／Windows interop／……）、設定路徑，以及執行中的處理程序數量。

**Hermes** —— 執行檔、資料目錄、虛擬環境、設定、閘道與儀表板狀態、MCP 伺服器
數量、連接埠、使用者服務與記錄路徑。
...
**連接埠** —— 通訊協定、位址、連接埠、PID、處理程序、監聽狀態，以及來源：
`WSL`、`Windows`，或真的能從 Windows 連到時的 `WSL + Windows`（並附上持有它的
Windows 處理程序）。只存在於 Windows 的監聽者也包含在內。
...
**Console** —— 目前的發行版、目前的目錄，以及 shell 狀態（就緒、執行中、等待
輸入、等待 sudo 密碼、已中斷連線）。

**透過 MCP** —— 以上全部，都能透過 26 個唯讀的 `Get*` 工具取得。
...
接下來（0.1.3）是診斷版：說明一個連接埠*為什麼*連不上（實際生效的網路模式、
繫結位址、預設就開著的 Hyper-V 防火牆）、標示專案落在慢速 `/mnt` 界線的哪一側、
指出那些其實悄悄指向 Windows 執行檔的開發工具、顯示時鐘偏差、加上趨勢走勢圖、
提供錯誤回報與 AGENTS.md 的複製範本，並在 Explorer 窗格裡顯示以目錄為單位的磁碟
使用量。再之後：適合代理程式使用的 MCP 工具、資源回收筒還原介面、服務記錄
檢視器、ARM64 版本、簽章過的安裝程式。
