...
還有發行版是否仍在回應：`wsl --list`在發行版停止回應後還會連續幾小時說它在執行，
所以當探測拿不到回應時，徽章會變成**執行中 — 無回應**並寫明最後一次回應的時間，
因為從那一刻起這裡的每個值都是最後一次的正常值，而不是新值。
...
一次編輯還早啟動時出現的橫幅提示。
還有 WSL 分散在兩台機器上的兩個答案：核心是否真的持有 `[interop] enabled=`
所要求的 interop 註冊 —— 該檔案只在發行版啟動時讀取一次，之後的修改在
`wsl --shutdown` 之前不起任何作用 —— 以及這個發行版以哪個使用者登入，而
Windows 登錄檔的 `DefaultUid` 會悄悄壓過 `/etc/wsl.conf` 裡的
`[user] default=`。
...
**開發者設定檔** —— 五套建議工具集（Web、Python、Rust、AI、容器 / Kubernetes）
與上方的已安裝工具目錄進行對照：已安裝多少、缺少多少，以及每個工具為何在清單中的一句說明。
缺少的工具會解析到 WSLPad 已在發行版中看到的套件管理員 —— 與 Discover 準備的相同的
apt / npm / cargo / Homebrew / snap 指令 —— 若都沒有則交給 Discover。**準備缺少的安裝**
只會在 Console 中放入一行可檢閱的指令，按下 Enter 之前什麼都不會執行。自訂設定檔
只是一個名稱加上從目錄中挑選的工具，儲存在設定中 —— WSLPad 不會變成套件管理員。
