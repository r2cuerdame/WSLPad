...
describe('parseObservations', () => {
  it('reads every probe out of one marker-separated round trip', () => {
    ...
    expect(obs.interopBinfmt).toBe('enabled')
    expect(obs.interopBinfmtLate).toBe('enabled')
    expect(obs.defaultUid).toBe(1000)
  })

  it('reads the two binfmt words the kernel writes, and nothing else', () => {
    const on = parseObservations(probeOutput({ binfmt: 'enabled', binfmtLate: 'disabled' }))
    expect(on.interopBinfmt).toBe('enabled')
    expect(on.interopBinfmtLate).toBe('disabled')
    // A missing node is a third state — no registration at all — not "off".
    const absent = parseObservations(probeOutput({}))
    expect(absent.interopBinfmt).toBeNull()
    expect(absent.interopBinfmtLate).toBeNull()
    // Anything that is not one of the two words stays unknown rather than
    // being coerced; a localized or truncated read must not become a verdict.
    const junk = parseObservations(probeOutput({ binfmt: 'aktiviert' }))
    expect(junk.interopBinfmt).toBeNull()
  })

  /**
   * Captured from a real WSL 2 machine: the binfmt node is not one word, it is
   * a whole record whose first line carries the state. Trimming the section as
   * a whole would compare against five lines and always miss.
   */
  it('reads the state out of the multi-line record the node really holds', () => {
    const node = ['enabled', 'interpreter /init', 'flags: P', 'offset 0', 'magic 4d5a'].join('\n')
    expect(parseObservations(probeOutput({ binfmt: node })).interopBinfmt).toBe('enabled')
    // -late is absent on most machines, which is unknown, not disabled.
    expect(parseObservations(probeOutput({ binfmt: node })).interopBinfmtLate).toBeNull()
  })

  it('reads uid 0 as root and a non-numeric answer as unknown', () => {
    expect(parseObservations(probeOutput({ uid: '0' })).defaultUid).toBe(0)
    expect(parseObservations(probeOutput({ uid: 'root' })).defaultUid).toBeNull()
    expect(parseObservations(probeOutput({})).defaultUid).toBeNull()
  })
...
describe('reconcileSettings distro side', () => {
  ...
  it('reads automount off the drive mounts, ignoring other Windows binds', () => {
    ...
    const { settings } = reconcileSettings(
      baseInput({
        observations: observations({ drvfsRoots: ['/mnt/c', '/mnt/d', '/Docker/host'] })
      })
    )
...
describe('createWslConfigCollector', () => {
  ...
  /** Injected everywhere, so no unit test ever shells out to reg.exe. */
  const REG_ENTRIES = [
    { distro: 'Ubuntu', basePath: null, vhdFileName: null, version: 2, defaultUid: 1000 }
  ]

  function collector(
    text: string | null,
    mtimeMs = NOW - 2 * HOUR,
    readRegistry: WslConfigDeps['readRegistry'] = async () => REG_ENTRIES
  ) {
  ...
  it('reports interop as the kernel has it, next to what the file declared', async () => {
    const probe = probeOutput({
      wslConf: '[interop]\nenabled=false',
      wslConfExists: '1',
      binfmt: 'enabled',
    ...
    const info = await collector(null).collect(makeRunner(probe), 'Ubuntu')
    // The file was edited after the distro started, so the kernel still has
    // the registration the old file asked for. Both halves are reported.
    expect(info.interop).toEqual({ binfmt: 'enabled', binfmtLate: 'enabled', declared: false })
  })

  it('leaves interop declared unknown when the word is not a boolean', async () => {
    const probe = probeOutput({ wslConf: '[interop]\nenabled=yes-please', wslConfExists: '1' })
    const info = await collector(null).collect(makeRunner(probe), 'Ubuntu')
    expect(info.interop?.declared).toBeNull()
    expect(info.interop?.binfmt).toBeNull()
  })

  it('reads the default user from inside the distro and from the registry', async () => {
    const probe = probeOutput({
      wslConf: '[user]\ndefault=dev',
      wslConfExists: '1',
      user: 'root',
    ...
    const info = await collector(null, NOW - 2 * HOUR, async () => [
      { distro: 'Ubuntu', basePath: null, vhdFileName: null, version: 2, defaultUid: 0 }
    ]).collect(makeRunner(probe), 'Ubuntu')
    expect(info.defaultUser).toEqual({
      effectiveUid: 0,
      effectiveName: 'root',
      registryUid: 0,
      declaredName: 'dev'
  ...
  it('keeps the registry uid unknown when the key cannot be read', async () => {
    const info = await collector(null, NOW - 2 * HOUR, async () => null).collect(
      makeRunner(PROBE),
      'Ubuntu'
    )
    expect(info.defaultUser?.registryUid).toBeNull()
    // The in-distro half still stands on its own.
    expect(info.defaultUser?.effectiveUid).toBe(1000)
    expect(info.defaultUser?.effectiveName).toBe('dev')
  })

  it('matches the registry entry case-insensitively, as wsl.exe does', async () => {
    const info = await collector(null, NOW - 2 * HOUR, async () => [
      { distro: 'ubuntu', basePath: null, vhdFileName: null, version: 2, defaultUid: 1000 }
    ]).collect(makeRunner(PROBE), 'Ubuntu')
    expect(info.defaultUser?.registryUid).toBe(1000)
  })

  it('leaves the path null when %USERPROFILE% cannot be resolved', async () => {
    const built = createWslConfigCollector({
      userProfile: null,
      now: () => NOW,
