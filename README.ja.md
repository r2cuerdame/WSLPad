...
**インストール済みツール** — 11 カテゴリ 112 種類のツール（AI CLI、ランタイム、
...
**開発者プロファイル** — 5 つの推奨ツールセット（Web、Python、Rust、AI、
コンテナ / Kubernetes）を上のインストール済みツールカタログと照合します:
いくつインストール済みで、いくつ不足しているか、そして各ツールがなぜそこにあるかを
一行で説明します。不足ツールは WSLPad がディストリビューション内で既に確認した
パッケージマネージャー — Discover が準備するのと同じ apt / npm / cargo /
Homebrew / snap コマンド — に解決され、どれも持っていなければ Discover に
引き渡されます。**不足分のインストールを準備**はレビュー可能な一行を Console に
置くだけで、Enter を押すまで何も実行されません。カスタムプロファイルは名前と
カタログから選んだツールだけで、設定に保存されます — WSLPad はパッケージ
マネージャーにはなりません。
