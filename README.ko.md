...
**설치된 도구** — 11개 분류에 걸친 112개 도구(AI CLI, 런타임, 패키지 관리자, 버전
...
**개발자 프로필** — 다섯 가지 권장 도구 세트(Web, Python, Rust, AI, 컨테이너 /
Kubernetes)를 위의 설치된 도구 카탈로그와 대조합니다: 몇 개가 설치되어 있고 몇
개가 빠져 있는지, 그리고 각 도구가 왜 거기 있는지 한 줄로 설명합니다. 빠진
도구는 WSLPad가 이미 배포판에서 확인한 패키지 관리자 — Discover가 준비하는 것과
같은 apt / npm / cargo / Homebrew / snap 명령 — 로 해석되고, 어느 것도 갖고
있지 않으면 Discover로 넘깁니다. **누락 설치 준비**는 검토 가능한 한 줄을
Console에 놓을 뿐이며 Enter를 누르기 전에는 아무것도 실행되지 않습니다. 사용자
지정 프로필은 이름과 카탈로그에서 고른 도구일 뿐이며 설정에 저장됩니다 —
WSLPad는 패키지 관리자가 되지 않습니다.
