...
**진단** — 이번 실행 중에 일어난 절전·재개, 배포판 응답 상태, DNS와 네트워크 모드
변경, Console 복구, 사용자가 직접 실행한 네트워크 검사를 하나의 타임라인으로
이어 줍니다. 검사는 WSL과 Windows의 DNS, WSL 기본 경로, 선택한 Windows localhost
포트를 비교합니다. 백그라운드에서는 네트워크를 검사하지 않으며, 진단 내보내기 전에
개인정보가 될 수 있는 필드를 먼저 알려 줍니다.
...
**진단** — 이번 실행 중 절전·재개, 배포판, DNS, 네트워크 모드, Console 상태 변화를
기록한 타임라인, 사용자가 직접 실행한 최신 WSL/Windows 네트워크 검사, 그리고 로컬
경로·호스트 정보·네트워크 주소가 들어감을 저장 전에 알리는 진단 JSON 묶음. 타임라인은
앱을 종료한 뒤 보존하지 않습니다.
