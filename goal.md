...
- **0.7.0: 고장 신호가 아예 없는 세 가지 고장.** 셋 다 "안 되는데 아무것도 안 되는 티를 내지 않는" 자리다. (1) `/mnt/c`에 `metadata` 옵션이 없으면 `chmod`와 `chown`이 **성공했다고 말하고 아무것도 저장하지 않는다** — 권한은 읽을 때마다 umask로 다시 만들어지므로 다음 `ls` 전에 이미 사라진다(§6.3.1). 파싱 쪽에도 함정이 있다: 현재 WSL 2에서 drvfs 옵션은 9p의 `aname=` 값 **안에 세미콜론으로** 들어가므로(`aname=drvfs;path=C:\;uid=1000;metadata`) 쉼표로만 쪼개는 뻔한 구현은 실기계에서 `metadata`를 영영 못 찾고 모든 드라이브를 "없음"으로 보고한다. 드라이브마다 실제 옵션을 보여주고, 파일이 `metadata`를 선언했는데 마운트에 없으면 `wsl --shutdown`이 필요하다고 말한다. (2) Defender 실시간 보호는 WSL이 `ext4.vhdx`에서 건드리는 모든 블록을 읽는다 — "WSL 디스크가 느리다"의 최대 원인인데 어느 화면도 둘을 잇지 않는다(§6.2.2). 여기서 가장 위험한 것은 순진한 구현이다: 권한 없이 부른 `Get-MpPreference`는 실패하지도, 빈 목록을 주지도 않고, 제외 항목 자리에 `"N/A: Must be an administrator to view exclusions"`라는 **문장 하나를 끼워 넣는다.** 그대로 믿으면 "제외 1개, 당신 것은 아님"이 되어 관리자가 아닌 모든 기계에서 자신 있게 틀린 답을 낸다. 그래서 문장을 읽지 않고 토큰의 승격 여부(`IsInRole`)를 먼저 보고, 진짜로 읽힌 경우가 아니면 목록은 `null`로 남긴다 — 없음이 아니라 모름이다. 제외 명령은 Console에 준비하지 않고 복사만 제공한다: Console은 배포판 안의 셸이고 이 명령은 Windows 관리자 권한이 필요하므로, 준비해 두면 Enter가 무엇을 할지에 대한 거짓말이 된다. (3) inotify watch가 바닥나면 커널은 ENOSPC를 돌려주고, vite·webpack·`tsc --watch`·VS Code가 모두 **"no space left on device"** 로 출력한다 — 꽉 차지도 않은 디스크를 지목하면서(§6.2.4). 두 한도를 보여주고 낮으면 그렇다고 말하며, 올리는 명령은 `sudo`가 아니라 `wsl -u root`로 준비한다(0.4.1). 소비량은 일부러 보여주지 않는다: 세려면 모든 프로세스의 `fdinfo`를 읽어야 하는데 일반 사용자에게는 `0`밖에 안 나오고, 여기서의 0은 데이터가 아니라 거짓말이다. MCP에 `GetDriveMounts`·`GetDefender`·`GetInotifyLimits`를 더해 40개가 되고, LLM 마크다운의 "함정" 절은 셋 다 명시한다.
- **0.8.0: 다 죽이지 않는 Remote Recovery.** 사용자가 직접 실행한 검사 하나가 배포판 응답·WSL/Windows DNS·기본 경로와 최신 프로세스 측정을 한 화면에 모은다. 일반 `node` 프로세스는 절대 편집기 서버로 추측하지 않고 명령줄에 `.vscode-server` 또는 `.vscode-server-insiders` 루트가 실제로 있는 프로세스만 역할별로 묶는다. WSL이 정상이고 서버 메인도 보이면 가장 작은 조치는 VS Code 창 Reload다. 서버 메인 없이 하위 프로세스만 남았다는 측정 근거가 있을 때만 그 PID들만 종료하는 명령을 권하며, 그다음은 선택한 배포판의 `wsl --terminate`, 모든 배포판·Docker·tmux에 영향을 주는 `wsl --shutdown`은 최후 수단이다. 절전 전 상태와 복귀 후 검사 시점의 배포판 응답·DNS·네트워크 모드·Console 차이도 함께 남긴다. 어떤 복구 명령도 자동 실행하지 않고 Console 입력에 준비만 한다.
...
## 6.2.4 파일 감시 한도 (0.7.0)

inotify watch를 다 쓰면 커널은 ENOSPC를 돌려주고, 그 위의 모든 도구가 "no space left on device"로 번역해 출력한다. 디스크는 멀쩡한데 디스크를 지목하는 에러다.

- `/proc/sys/fs/inotify/max_user_watches`와 `max_user_instances`를 보여준다. 큰 소스 트리에는 524288이 통상 권장값이며, 그 아래면 낮다고 말한다.
- **소비량은 보여주지 않는다.** 세려면 모든 프로세스의 `/proc/<pid>/fdinfo`를 읽어야 하고 일반 사용자는 자기 것 말고는 못 읽는다. 돌아오는 숫자는 0이고, 여기서 0은 데이터가 아니라 거짓말이다(§2.4).
- 올리는 명령은 `/etc/sysctl.d`에 전용 파일 하나를 쓰는 형태로 Console에 **준비만** 한다. 두 번 실행해도 같은 결과가 되도록 덧붙이지 않고 덮어쓴다. `sudo`가 아니라 `wsl -u root`로 간다 — 설치 관리자나 에이전트가 만든 배포판은 sudo 암호를 아무도 모르는 경우가 많다(0.4.1).
...
## 6.3.1 Windows 드라이브 마운트 (0.7.0)

`[automount] options=`는 의도이고, `/proc/mounts`는 실제다. 이 목록의 모든 경로가 어느 파일시스템 위에 있는지를 결정하므로 경로 목록보다 위에 둔다.

- 드라이브마다 실제 마운트 옵션을 보여준다: `metadata`, `case`, `uid`/`gid`, `umask`/`fmask`/`dmask`.
- **`metadata`가 없으면 `chmod`·`chown`이 성공했다고 말하고 아무것도 저장하지 않는다.** 권한은 읽을 때마다 umask로 합성되므로 다음 `ls` 전에 사라진다. 실행 비트가 남아야 하는 일은 배포판 자신의 파일시스템에서 하라고 말한다.
- 파싱은 두 세퍼레이터를 모두 쪼갠다. 현재 WSL 2에서 drvfs 옵션은 9p의 `aname=` 값 안에 **세미콜론으로** 들어간다(`aname=drvfs;path=C:\;uid=1000;metadata`). 쉼표로만 쪼개면 실기계에서 `metadata`를 한 번도 못 찾는다. WSL 1의 평범한 쉼표 목록도 같은 코드로 읽힌다.
- 자동 마운트 드라이브만 센다. Docker Desktop이 거는 Windows 폴더 바인드는 DrvFs지만 드라이브가 아니다.
- 파일이 `metadata`를 선언했는데 마운트에 없으면 선언과 실효의 불일치이며, 드라이브는 배포판 시작 시 마운트되므로 `wsl --shutdown`이 필요하다고 말한다.
